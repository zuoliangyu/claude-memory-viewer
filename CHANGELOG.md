...
- 修复 Codex 仅含元数据、没有可见活动的 rollout 被当作有效会话，导致主列表出现大量“（无标题）”条目的问题；空会话分类与有效会话列表现在共享同一次扫描和缓存。
- 修复 Grok 项目列表与会话列表在同一轮刷新中重复解析全部历史文件的问题；两者现在共享进程内快照，并在文件变化、删除或显式刷新时失效。
